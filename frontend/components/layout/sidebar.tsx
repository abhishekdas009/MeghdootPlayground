  ...
  const navItems = (onNavigate?: () => void) =>
    NAV_ITEMS.map((item) => {
      ...
      return (
        ...
        >
          {/* INCREASED ICON SIZE HERE: Changed h-5 w-5 to h-6 w-6 */}
          <Icon className={cn("h-6 w-6 shrink-0", isActive ? "text-accent" : "text-muted-foreground group-hover:text-foreground")} />
          <motion.span
            initial={false}
    ...
    <>
      ...
      >
        ...
        <div className="border-t border-border p-2">
          ...
          >
            {/* INCREASED COLLAPSE TOGGLE SIZE: Changed 16x16 to 20x20 */}
            <svg
              width="20"
              height="20"
              viewBox="0 0 24 24"
            ...
            >
              <div className="flex min-h-16 items-center justify-between border-b border-border px-3">
                ...
                <button
                  ...
                  aria-label="Close navigation"
                >
                  {/* INCREASED MOBILE CLOSE ICON SIZE: Changed h-5 w-5 to h-6 w-6 */}
                  <X className="h-6 w-6" />
                </button>
              </div>
