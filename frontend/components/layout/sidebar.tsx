...
export function Sidebar() {
  ...
  const renderNavItems = (onNavigate?: () => void, isMobile = false) =>
    NAV_ITEMS.map((item) => {
      ...
      return (
        <React.Fragment key={item.href}>
          ...
          >
            ...
            <div
              className={cn(
                "sidebar-nav-icon flex h-8 w-8 shrink-0 items-center justify-center rounded-xl transition-all duration-200 group-hover:scale-110",
                isActive
                  ? iconBg
          ...
          <>
            ...
            >
              ...
              <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] no-scrollbar">
                {renderNavItems(() => setMobileSidebarOpen(false), true)}
              </nav>
