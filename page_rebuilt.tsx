...
export default function SOQLGeneratorPage() {
  ...
  const handleProcessTransfer = () => {
    ...
    for (const pair of assetPairs) {
      ...
      if (!assetRow) {
        ...
        debugLines.push(`Γ¥î ${pair.componentId} ΓåÆ Asset not found in Component SOQL result`);
        continue;
      }
      ...
      if (!accountId) {
        ...
        debugLines.push(`Γ¥î ${pair.componentId} ΓåÆ CID ${pair.newCid} not found in Account SOQL result`);
        continue;
      }

      if (!assetId) {
        debugLines.push(`Γ¥î ${pair.componentId} ΓåÆ No valid asset ID found`);
        continue;
      }

      rows.push(`"[Asset]","${assetId}","${accountId}"`);
      debugLines.push(`Γ£à ${pair.componentId} ΓåÆ ${assetId} (${sourceNote}) | CID ${pair.newCid} ΓåÆ ${accountId}`);
    }
  ...
  return (
    <div className="workspace-page mx-auto w-full max-w-7xl space-y-6 pb-14 p-4 sm:p-6 lg:space-y-8 lg:p-8">
      {/* ΓöÇΓöÇΓöÇ Header Section ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <motion.div
            ...
            <CardHeader className="pb-4 bg-transparent relative z-10 p-6">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Badge className="text-[10px] font-black bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full whitespace-nowrap text-slate-500 border border-slate-200 dark:border-slate-700 shadow-sm uppercase tracking-widest">
                    {defaultTemplateCount} Built-in
                  </Badge>
                  <Badge className={cn("text-[10px] font-black px-3 py-1 rounded-full whitespace-nowrap shadow-sm uppercase tracking-widest border", libraryLoadState === "error" ? "bg-red-500/10 text-red-600 border-red-500/20" : "bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20")}>
