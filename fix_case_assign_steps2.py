...
# Replace Assignment Mode Header
pattern_step2 = re.compile(r'<CardHeader className="pb-4 bg-transparent p-5 relative z-10">\s*<div className="flex items-center justify-between gap-3 flex-wrap">\s*<div className="flex items-center gap-3 flex-1">\s*<div className="flex h-9 w-9 items-center justify-center rounded-xl bg-purple-500/10 text-purple-600 dark:text-purple-400 shadow-inner">\s*<CheckCircle2 className="h-4\.5 w-4\.5" />\s*</div>\s*<CardTitle className="text-sm font-black tracking-tight text-foreground flex-1">Assignment Mode &amp; Execution</CardTitle>\s*</div>.*?</div>\s*</CardHeader>', re.DOTALL)
...
if pattern_step2.search(page):
    page = pattern_step2.sub(new_step2, page)
    print("Replaced Case Assign Step 2")
else:
    print("Failed to match Step 2")

# Replace Final Assignment Output Header
pattern_step3 = re.compile(r'<CardHeader className="pb-4 bg-transparent p-5 relative z-10">\s*<div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">\s*<div className="flex items-center gap-4">\s*<div className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 shadow-inner">\s*<CheckCircle2 className="h-4\.5 w-4\.5" />\s*</div>\s*<div>\s*<CardTitle className="text-sm font-black tracking-tight text-foreground">Final Assignment Output</CardTitle>.*?</div>\s*</CardHeader>', re.DOTALL)
